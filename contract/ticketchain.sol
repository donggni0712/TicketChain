...
contract IKIP17Metadata is IKIP17 {
    ...
    function ticketExpired(uint256 tokenId) external view returns (uint256);

    function ticketPlaceName(uint256 tokenId)
        external
        view
...
contract KIP17Metadata is KIP13, KIP17, IKIP17Metadata {
     ...
     *     bytes4(keccak256('ticketName(uint256)')) == 0xc87b56dd
     *
    ...
    function ticketExpired(uint256 tokenId) external view returns (uint256) {
        ...
        return _ticketExpireds[tokenId];
    }
     ...
     */
    function _setticketName(uint256 tokenId, string memory uri) internal {
        require(
            _exists(tokenId),
            "KIP17Metadata: URI set of nonexistent token"
        );
        _ticketNames[tokenId] = uri;
    }

    function _setexpired(uint256 tokenId, uint256 uri) internal {
        require(
            _exists(tokenId),
            "KIP17Metadata: URI set of nonexistent token"
        );
        _ticketExpireds[tokenId] = uri;
    }

    function _setplaceName(uint256 tokenId, string memory uri) internal {
        require(
            _exists(tokenId),
            "KIP17Metadata: URI set of nonexistent token"
        );
...
contract KIP17MetadataMintable is KIP13, KIP17, KIP17Metadata, MinterRole {
    /*
     *     bytes4(keccak256('mintWithticketName(address,uint256,string)')) == 0x50bb4e7f
