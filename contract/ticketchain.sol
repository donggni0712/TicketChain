...
// File: contracts/ticket/KIP17/IKIP17.sol
...
// File: contracts/ticket/KIP17/IERC721Receiver.sol

pragma solidity ^0.5.0;

/**
 * @title ERC721 ticket receiver interface
...
// File: contracts/ticket/KIP17/IKIP17Receiver.sol

pragma solidity ^0.5.0;

/**
 * @title KIP17 ticket receiver interface
 * @dev Interface for any contract that wants to support safeTransfers
 * from KIP17 asset contracts.
 * @dev see http://KIPs.klaytn.com/KIPs/KIP-17-non_fungible_ticket
 */
...
// File: contracts/ticket/KIP17/KIP17.sol

pragma solidity ^0.5.0;

/**
 * @title KIP17 Non-Fungible ticket Standard basic implementation
 * @dev see http://KIPs.klaytn.com/KIPs/KIP-17-non_fungible_ticket
 */
contract KIP17 is KIP13, IKIP17 {
    ...
    mapping(address => Counters.Counter) private _ownedticketsCount;
    ...
    function balanceOf(address owner) public view returns (uint256) {
        ...
        return _ownedticketsCount[owner].current();
    }
    ...
    /**
     * @dev Approves another address to transfer the given ticket ID
     ...
     * An operator is allowed to transfer all tickets of the sender on their behalf.
    ...
    /**
     * @dev Safely transfers the ownership of a given ticket ID to another address
    ...
    /**
     * @dev Internal function to transfer ownership of a given ticket ID to another address.
...
// File: contracts/ticket/KIP17/IKIP17Enumerable.sol

pragma solidity ^0.5.0;

/**
 * @title KIP-17 Non-Fungible ticket Standard, optional enumeration extension
 * @dev See http://KIPs.klaytn.com/KIPs/KIP-17-non_fungible_ticket
 */
...
// File: contracts/ticket/KIP17/KIP17Enumerable.sol

pragma solidity ^0.5.0;

/**
 * @title KIP-17 Non-Fungible ticket with optional enumeration extension logic
 * @dev See http://KIPs.klaytn.com/KIPs/KIP-17-non_fungible_ticket
 */
contract KIP17Enumerable is KIP13, KIP17, IKIP17Enumerable {
    ...
    // Mapping from ticket ID to index of the owner tickets list
    mapping(uint256 => uint256) private _ownedticketsIndex;

    // Array with all ticket ids, used for enumeration
    uint256[] private _alltickets;

    // Mapping from ticket id to position in the alltickets array
    mapping(uint256 => uint256) private _allticketsIndex;

    /*
     *     bytes4(keccak256('totalSupply()')) == 0x18160ddd
     *     bytes4(keccak256('ticketOfOwnerByIndex(address,uint256)')) == 0x2f745c59
     *     bytes4(keccak256('ticketByIndex(uint256)')) == 0x4f6ccce7
     *
    ...
    /**
     * @dev Gets the ticket ID at a given index of the tickets list of the requested owner.
     * @param owner address owning the tickets list to be accessed
     * @param index uint256 representing the index to be accessed of the requested tickets list
     * @return uint256 ticket ID at the given index of the tickets list owned by the requested address
     */
    function ticketOfOwnerByIndex(address owner, uint256 index)
        public
        view
    ...
    /**
     * @dev Gets the total amount of tickets stored by the contract.
     * @return uint256 representing the total amount of tickets
     */
    function totalSupply() public view returns (uint256) {
    ...
    /**
     * @dev Gets the ticket ID at a given index of all the tickets in this contract
     * Reverts if the index is greater or equal to the total number of tickets.
     * @param index uint256 representing the index to be accessed of the tickets list
     * @return uint256 ticket ID at the given index of the tickets list
     */
    function ticketByIndex(uint256 index) public view returns (uint256) {
        require(
            index < totalSupply(),
    ...
    /**
     * @dev Gets the list of ticket IDs of the requested owner.
     * @param owner address owning the tickets
     * @return uint256[] List of ticket IDs owned by the requested address
     */
    function _ticketsOfOwner(address owner)
        internal
        view
        returns (uint256[] storage)
    {
        return _ownedtickets[owner];
    }
    ...
    /**
     * @dev Private function to remove a ticket from this extension's ownership-tracking data structures. Note that
     * while the ticket is not assigned a new owner, the _ownedticketsIndex mapping is _not_ updated: this allows for
...
// File: contracts/ticket/KIP17/IKIP17Metadata.sol

pragma solidity ^0.5.0;

/**
 * @title KIP-17 Non-Fungible ticket Standard, optional metadata extension
 * @dev See http://KIPs.klaytn.com/KIPs/KIP-17-non_fungible_ticket
 */
...
contract KIP17Metadata is KIP13, KIP17, IKIP17Metadata {
     ...
     *     bytes4(keccak256('ticketURI(uint256)')) == 0xc87b56dd
     *
    ...
    /**
     * @dev Gets the ticket name.
     * @return string representing the ticket name
     */
    function name() external view returns (string memory) {
        return _name;
    }
    ...
    /**
     * @dev Gets the ticket symbol.
     * @return string representing the ticket symbol
     */
    function symbol() external view returns (string memory) {
        return _symbol;
    }
...
// File: contracts/ticket/KIP17/KIP17Full.sol

pragma solidity ^0.5.0;

/**
 * @title Full KIP-17 ticket
...
// File: contracts/ticket/KIP17/KIP17MetadataMintable.sol
...
contract KIP17MetadataMintable is KIP13, KIP17, KIP17Metadata, MinterRole {
    /*
     *     bytes4(keccak256('mintWithticketURI(address,uint256,string)')) == 0x50bb4e7f
...
// File: contracts/ticket/KIP17/KIP17Mintable.sol
...
// File: contracts/ticket/KIP17/KIP17Burnable.sol

pragma solidity ^0.5.0;

/**
 * @title KIP17 Burnable ticket
 * @dev KIP17 ticket that can be irreversibly burned (destroyed).
 * See http://KIPs.klaytn.com/KIPs/KIP-17-non_fungible_ticket
 */
...
// File: contracts/ticket/KIP17/KIP17Pausable.sol

pragma solidity ^0.5.0;

/**
 * @title KIP17 Non-Fungible Pausable ticket
...
// File: contracts/ticket/KIP17/KIP17Ticket.sol
